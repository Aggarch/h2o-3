...
//      buildModel();
      ...
      if (!_parms._lambda_search & !_parms._HGLM)
        updateProgress(false);
      double bestDevianceTrain = Double.MAX_VALUE;
      double bestDevianceTest = Double.MAX_VALUE;
      ...
      for (int alphaInd = 0; alphaInd < _parms._alpha.length; alphaInd++) {
        ...
        for (int i = 0; i < _parms._lambda.length; ++i) {  // lambda search
          ...
          devHistoryTest[_model._output._selected_submodel_idx % devHistoryTest.length] = (oldDevTest - testDev) / oldDevTest; // only remembers 5
          oldDevTest = testDev;
          devHistoryTrain[_model._output._selected_submodel_idx % devHistoryTrain.length] = (oldDevTrain - trainDev) / oldDevTrain;
